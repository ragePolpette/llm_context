...
def assemble_functional_context(
    *,
    ...
    entry_points = _build_entry_points(symbol_results, limit=opts.max_entry_points)
...
def _build_entry_points(symbol_results: list[dict[str, Any]], limit: int) -> list[dict[str, Any]]:
    ranked = sorted(
        symbol_results,
        key=lambda item: (
            _symbol_kind_rank(str(item.get("kind") or "")),
...
def _render_assembled_context(core_files: list[dict[str, Any]], max_chars: int) -> str:
    ...
    for file_item in core_files:
        lines = [
            ...
            f"aggregate_score={file_item['aggregate_score']:.4f} max_score={file_item['max_score']:.4f}",
        ]
